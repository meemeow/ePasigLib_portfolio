...
import type { CollectionViewTab } from "@/features/lms/collections/pages/view-collection/types/collections-view-types";

interface ViewSidebarProps {
  activeTab: CollectionViewTab;
  onTabChange: (tab: CollectionViewTab) => void;
  onReturn: () => void;
  actions?: ReactNode;
}
...
export default function ViewSidebar({
  ...
  const tabs: Array<{ value: CollectionViewTab; label: string }> = [
    { value: "home", label: "Home" },
    { value: "marc", label: "MARC" },
    { value: "modification_logs", label: "Modification Logs" },
    { value: "checkout_history", label: "CheckOut History" },
    { value: "checkin_history", label: "CheckIn History" },
  ];
  ...
  return (
    <div className="flex w-full flex-col gap-2 pt-5 pb-3 sm:grid sm:grid-cols-2 sm:items-center sm:gap-x-0 sm:gap-y-2 xl:flex xl:w-56 xl:flex-col xl:items-stretch xl:justify-start xl:space-y-4 xl:p-2">
      <Button
        variant="outline"
        onClick={onReturn}
        className={`order-2 mb-2 w-full shrink-0 sm:order-none sm:col-start-1 sm:row-start-2 sm:justify-self-start sm:w-[150px] md:w-[175px] lg:w-[230px] xl:order-2 xl:w-full ${COMPACT_CONTROL}`}
      >
        ← Return
      </Button>
      ...
      <div className="order-4 w-full sm:col-start-2 sm:row-start-1 sm:w-auto sm:justify-self-end xl:order-3 xl:mt-4 xl:w-full">
        <div className="block xl:hidden">
          <Select
            value={activeTab}
            onValueChange={(value) => onTabChange(value as CollectionViewTab)}
          >
