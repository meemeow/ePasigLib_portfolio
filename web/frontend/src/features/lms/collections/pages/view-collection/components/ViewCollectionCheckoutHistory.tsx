...
import { formatTimestamp, formatTimestampDate } from "@/lib/format/date";
import type { CheckoutHistory } from "@/features/lms/collections/pages/view-collection/types/collections-view-types";

interface ViewCollectionCheckoutHistoryProps {
...
export default function ViewCollectionCheckoutHistory({
  history,
  loading,
  onRefresh,
  currentPage,
  ...
  onNextPage,
  onPrevPage,
}: ViewCollectionCheckoutHistoryProps) {
  return (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <Text className="text-xs md:text-sm text-gray-500">
                View all checkout history for this collection.
              </Text>
            </div>
            <Button
              variant="outline"
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="px-3 py-2 border text-left">Return Status</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading && history.length === 0 ? (
              ...
              history.map((entry, idx) => (
                <TableRow key={entry.id ?? idx} className="bg-white">
                  ...
                  <TableCell className="px-3 py-2 border">{entry.CheckedOutBy || "-"}</TableCell>
                  <TableCell className="px-3 py-2 border">{formatTimestamp(entry.CheckoutDate)}</TableCell>
                  <TableCell className="px-3 py-2 border">{formatTimestampDate(entry.DueDate)}</TableCell>
