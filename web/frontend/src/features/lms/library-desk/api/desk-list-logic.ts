...
import { fetchDeskRowsApi } from "@/features/lms/library-desk/api/desk-api";
import {
  clearSectionSearchCache,
  sectionSearchCache,
  UPDATES_CACHE_TTL,
  UPDATES_DEBOUNCE_DELAY,
} from "@/features/lms/library-desk/api/desk-records-cache";
import {
  buildDeskFiltersPayload,
  buildDeskSortPayload,
...
export function useDeskRecords<S extends DeskSection>(section: S) {
  ...
  const [sortBy, setSortBy] = useState<DeskSortState<S>>({
    ...config.defaultSort,
  });
  const [pendingSort, setPendingSort] = useState<DeskSortState<S>>({
    ...config.defaultSort,
  });
  ...
  const getCacheKey = useCallback(
    (
      ...
      sortState: DeskSortState<S> = sortBy,
  ...
  const applySuppression = useCallback((incoming: SectionRow[S][]) => {
    ...
    return incoming.filter((row) => !suppressedIdsRef.current.has(row.id));
  }, []);
  ...
  const fetchRecords = useCallback(
    async (
      ...
      const payload: FetchDeskPayload = {
        section,
        limit: itemsPerPage,
        page: Math.max(1, page),
        searchTerm: (opts?.searchTerm ?? "").trim() || undefined,
        filters: buildDeskFiltersPayload(section, filters),
        sortBy: buildDeskSortPayload(sortBy),
        ...(section === "conversations" ? { staffCode: staffCode ?? "" } : {}),
      };

      const response = await fetchDeskRowsApi<S>(payload, opts?.isRefresh);

      if (seq !== requestSeqRef.current || !mountedRef.current) return undefined;
      ...
      if (response) {
        ...
        setErrorMessage("Could not load updates. Try Refresh.");
      }

      setTableLoading(false);
      return response;
    },
    [section, itemsPerPage, filters, sortBy, applySuppression, staffCode],
  );
  ...
  const performSearch = useCallback(
    async (term: string): Promise<void> => {
      ...
      if (cached && Date.now() - cached.timestamp < UPDATES_CACHE_TTL) {
...
export type DeskRecordsHook<S extends DeskSection> = ReturnType<
  typeof useDeskRecords<S>
>;
