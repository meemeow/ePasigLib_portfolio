...
import type { ModificationLog } from "@/features/lms/profile/types/profile-types";

interface ProfileModificationLogsProps {
...
function formatLogDate(on: ModificationLog["On"]): string {
  if (!on) return "";
  const date = new Date(on);
  return Number.isNaN(date.getTime()) ? String(on) : date.toLocaleString();
}

export default function ProfileModificationLogs({
  logs,
  loading,
  currentPage,
  totalPages,
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <Text className="text-xs text-gray-500 md:text-sm">
                Every change made to your account, and who made it.
              </Text>
            </div>
            <Button
              variant="outline"
          ...
          <TableBody>
            {loading ? (
              <TableEmptyRow colSpan={6}>
                <Loader2 className="mx-auto h-6 w-6 animate-spin text-[#128CF1]" />
              </TableEmptyRow>
            ) : logs.length === 0 ? (
              <TableEmptyRow colSpan={6}>No modification logs available</TableEmptyRow>
            ) : (
              logs.map((log, index) => (
                <TableRow key={`${log.Action}-${index}`} className="bg-white">
                  ...
                  <TableCell className="border px-3 py-2 whitespace-normal">
                    {log.Description}
                  </TableCell>
                  <TableCell className="border px-3 py-2">{log.By}</TableCell>
                  <TableCell className="border px-3 py-2">
                    {formatLogDate(log.On)}
                  </TableCell>
                </TableRow>
              ))
            )}
