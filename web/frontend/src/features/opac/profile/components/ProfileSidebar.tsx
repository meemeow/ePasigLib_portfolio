...
import type { ProfileTab } from "@/features/opac/profile/types/opac-profile-types";
...
const tabs: Array<{ value: ProfileTab; label: string }> = [
  ...
  { value: "visit_logs", label: "Visit Logs" },
];
            ...
            <SelectContent>
              {tabs.map((tab) => (
                <SelectItem key={tab.value} value={tab.value}>
                  {tab.label}
                  {tab.value === "id_verification" && needsAttention
                    ? " (Action needed)"
                    : ""}
                </SelectItem>
              ))}
        ...
        <div className="hidden xl:flex flex-col gap-1">
          {tabs.map((tab) => (
            <Button
              ...
              className={`flex w-full items-center justify-between whitespace-normal font-normal gap-2 rounded-lg px-4 py-3 text-left text-sm font-[gothamMedium] transition ${
                activeTab === tab.value
